...
for random_seed in tqdm(random_seeds, desc="Imputing with random seeds"):
    ...
    linear_test = denormalise_data(filled_data_test, mins_test, maxs_test)
    linear_covid = denormalise_data(filled_data_covid, mins_covid, maxs_covid)

    # Time series plot for test set
    plt.figure(figsize=(12, 6))
    column = column_names_test[first_index_test]
    plt.plot(linear_test.index, linear_test[column], label=f"Linear model", alpha=1, color="m")
    # Replace graph-propagated values into the filled data from linear interpolation
    ...
    plt.plot(linear_covid.index, linear_covid[column], label=f"Linear model", alpha=1, color="m")
